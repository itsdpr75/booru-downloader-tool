...
def get_user_input():
    ...
    file_link_selector = input("Selector CSS para los enlaces de archivos (ej. 'a.file-link'): ")
    file_url_attribute = input("Atributo del enlace que contiene la URL del archivo (ej. 'href'): ")
    download_link_selector = input("Selector CSS para el enlace de descarga en la página del archivo (ej. 'a.download-link'): ")
...
def get_total_pages(url, page_param):
    ...
    # Buscar números en los enlaces de paginación
...
def get_download_url(file_page_url, download_link_selector, base_url):
    ...
    soup = BeautifulSoup(response.text, 'html.parser')
    download_link = soup.select_one(download_link_selector)
...
def main():
    ...
    for page in range(1, total_pages + 1):
        page_url = f"{config['search_url']}{config['page_param']}{page}"
